...
def extract_text_from_single_file(json_file, target_language, clf, skip_cleaning=True, linebreaks=True):
    min_block_length = 10
    ...
    # Note: updated to match recommendations from EDA (cleaning after correlating)
...
def correlate_and_clean_text(text_fr, text_en, pub_number, sentence_encoder, device, linebreaks=True):
    sentences_fr, sentences_en = create_sentences(text_fr, text_en, linebreaks=linebreaks)
...
def process_row(row_tuple, device, language_classifier, sentence_encoder, skip_abstract_only_translations=False,
    ...
    if filename_fr == filename_en:
        text_fr, text_en = extract_both_languages_from_single_file(fr_link, language_classifier, linebreaks=linebreaks)
    else:
        en_link = get_json_file_link(parsed_docs_folder, filename_en)
        if en_link is None:
            return None
        text_fr, text_en = extract_both_languages_from_two_files(fr_link, en_link, language_classifier, linebreaks=linebreaks)
    ...
    return correlate_and_clean_text(text_fr, text_en, pub_number, sentence_encoder, device, linebreaks=linebreaks)
